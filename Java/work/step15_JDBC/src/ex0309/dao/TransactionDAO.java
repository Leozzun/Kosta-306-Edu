   ...
   * */
	public void transfer(String inputAccount, String outputAccount, int money) {
		...
		try {
			...
			if(result==0) throw new SQLException("출금계좌 오류로 계좌이체를 실패하였습니다.");
		
			//입금하기
			result = this.deposit(con, inputAccount, money);
			if(result==0) throw new SQLException("입금계좌 오류로 계좌이체를 실패하였습니다.");
		
			//잔액 확인
			if(this.balanceCheck(con, inputAccount)) {
				throw new SQLException("입금계좌 총 금액 초과로 계좌이체를 실패하였습니다.");
			}
